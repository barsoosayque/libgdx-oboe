...
int64_t asset_seek(void *opaque, int64_t offset, int whence) {
    auto asset = static_cast<std::shared_ptr<AAsset> *>(opaque);
    switch (whence) {
        case AVSEEK_SIZE:return AAsset_getLength64(asset->get());
        case AVSEEK_FORCE:debug("Force seek requested. Skipping...");
            return -1;
        default:return AAsset_seek64(asset->get(), offset, whence);
    }
}
}
