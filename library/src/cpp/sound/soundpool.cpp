...
void soundpool::do_by_id(long id, std::function<void(std::vector<soundpool::sound>::iterator)> callback) {
...
soundpool::sound soundpool::gen_sound(float volume, float pan, float speed, bool loop) {
    return sound {
        ...
        .m_resampler = resampler(resampler::converter::zero_order_hold, m_channels, 1.f / std::clamp(speed, 0.5f, 2.0f))
    };
}
